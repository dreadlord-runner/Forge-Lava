  ...
  const features = [
    {
      ...
      desc: "Spin up servers, deploy with zero downtime, and ship without SSL headaches.",
    },
    {
      icon: (
      ...
      desc: "Choose your cloud, or use Laravel VPS. Forge handles it all, while you keep root access to every server.",
    },
    {
      icon: <Zap className="w-6 h-6 text-[#46c5af] mb-6" strokeWidth={2.5} />,
      title: "Crafted for productivity",
      desc: "Clean UI with the fewest clicks and toggles. Move faster from code to production.",
    },
  ];
  return (
    <section>
      ...
      <div className="flex items-stretch bg-innerBorderGray">
        ...
        <div className="flex-1 lg:px-8 py-2 md:py-2.5 flex items-center space-x-3 font-mono text-xs">
      ...
      <div className="w-full bg-white border-y border-gray-100">
        <div className="max-w-7xl mx-auto border-x border-gray-100">
          {/*-------------Hero Content------------ */}
          <div className="flex py-12 md:py-20 lg:py-24 px-4 sm:px-8 md:px-0">
          ...
          <div className="bg-white border-y border-gray-100">
            <div className="flex flex-row max-w-7xl mx-auto">
              <div className="hidden md:block border-r border-gray-100 w-20 shrink-0" />
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
                {features.map(({ icon, title, desc }) => (
                  <div
                    key={title}
                    className="py-10 px-6 sm:px-8 flex-col items-start"
                  >
                    <div className="mb-6">{icon}</div>
                    <div className="text-xl text-gray-900 mb-3">{title}</div>
                    <div className="text-gray-500 font-light">{desc}</div>
                  </div>
                ))}
              </div>
              <div className="hidden md:block border-l border-gray-100 w-20 shrink-0" />
            </div>
          </div>
